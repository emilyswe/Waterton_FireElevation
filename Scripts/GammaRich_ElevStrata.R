...
for (q in 1:hml) {
  ...
  location_data$severity[q] <- emda4$grid.code[which(emda4$location == location_data$location[q])[1]]  # Burn status
}
...
# Combine altitude bins and burn severity for strata definition
...
# Label definitions for strata (ensuring matching for strata labels)
...
write.csv(gari_stats, "Output/09_gamma_richness_ElevStrata.csv", row.names = FALSE)
...
# Save the plots
ggsave("Output/09_gamma_richness_low_unburned_elev_strata_only.png", plot = plot_low_unburned, width = 8, height = 6)
ggsave("Output/09_gamma_richness_mid_unburned_elev_strata_only.png", plot = plot_mid_unburned, width = 8, height = 6)
ggsave("Output/09_gamma_richness_high_unburned_elev_strata_only.png", plot = plot_high_unburned, width = 8, height = 6)
ggsave("Output/09_gamma_richness_low_burned_elev_strata_only.png", plot = plot_low_burned, width = 8, height = 6)
ggsave("Output/09_gamma_richness_mid_burned_elev_strata_only.png", plot = plot_mid_burned, width = 8, height = 6)
ggsave("Output/09_gamma_richness_high_burned_elev_strata_only.png", plot = plot_high_burned, width = 8, height = 6)
...
generate_comparative_plot <- function(data, unburned_strata, burned_strata) {
  ggplot() +
    geom_ribbon(data = data[data$strata == unburned_strata, ], aes(x = numpc, ymin = l95, ymax = u95, fill = "Unburned"), alpha = 0.5) +
    geom_line(data = data[data$strata == unburned_strata, ], aes(x = numpc, y = mean_richness, color = "Unburned")) +
    geom_ribbon(data = data[data$strata == burned_strata, ], aes(x = numpc, ymin = l95, ymax = u95, fill = "Burned"), alpha = 0.5) +
    geom_line(data = data[data$strata == burned_strata, ], aes(x = numpc, y = mean_richness, color = "Burned")) +
...
ggsave("Output/09_gamma_richness_low_comparison_elev_strata_only.png", plot = plot_low_comparison, width = 8, height = 6)
ggsave("Output/09_gamma_richness_mid_comparison_elev_strata_only.png", plot = plot_mid_comparison, width = 8, height = 6)
ggsave("Output/09_gamma_richness_high_comparison_elev_strata_only.png", plot = plot_high_comparison, width = 8, height = 6)
